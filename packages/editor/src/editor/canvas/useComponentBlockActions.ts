  ...
  const duplicateElement = () => {
    setData(
      oldData =>
        modifyData(oldData, {
          type: 'paste',
          data: { componentName: data.type, clipboard: data.config, targetId: data.cid, componentByName }
        }).newData
    );
  };
  ...
  const createActionButton = () => {
    setData(
      oldData =>
        modifyData(oldData, {
          type: 'add',
          data: { componentName: 'Button', targetId: COLUMN_DROPZONE_ID_PREFIX + data.cid, componentByName }
        }).newData
    );
  };
  ...
  const createElement = (name: ComponentType) => {
    setData(
      oldData =>
        modifyData(oldData, {
          type: 'add',
          data: { componentName: name, targetId: creationTargetId(oldData.components, data.cid), componentByName }
        }).newData
    );
  };
