...
test('create from data', async ({ page }) => {
  const editor = await FormEditor.openForm(page, { file: 'dialog/form/test/project/test/test.f.json' });
...
test('cms browser', async ({ page }) => {
  const editor = await FormEditor.openForm(page, { file: 'dialog/form/test/project/free/free.f.json' });
