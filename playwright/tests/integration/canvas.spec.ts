...
test.describe('empty', () => {
  ...
  test('init data', async ({ page }) => {
    const { canvas } = await FormEditor.openForm(page, { file: 'src_hd/form/test/project/empty/empty.f.json' });
    const dialog = await canvas.openInitCreateDialog();
...
test.describe('quickbar', () => {
  ...
  test('change', async ({ page }) => {
    ...
    await editor.canvas.blockByText('Firstname').inscribe();
...
test('icon fonts', async ({ page }) => {
  ...
  await expect(canvas.locator.locator('.ti.ti-check').first()).toBeVisible();
  await expect(canvas.locator.locator('.ti.ti-x').first()).toBeVisible();
});
